...
import com.tch.manager.FileManager;
import com.tch.manager.upload.FilePictureUpload;
import com.tch.manager.upload.PictureUploadTemplate;
import com.tch.manager.upload.UrlPictureUpload;
import com.tch.mapper.PictureMapper;
...
public class PictureServiceImpl extends ServiceImpl<PictureMapper, Picture> implements PictureService {
    ...
    public PictureVO uploadPicture(Object inputSource, PictureUploadRequest pictureUploadRequest, User loginUser) {
        ...
        if (pictureId != null) {
            ...
            // 校验空间是否一致
            if (spaceId == null) {
                // 如果没传 spaceId，则复用原有图片的spaceId
                if (oldPicture.getSpaceId() != null) {
                    spaceId = oldPicture.getSpaceId();
                }
            } else {
                // 如果传了 spaceId，必须和原有图片一致
    ...
    public void doPictureReview(PictureReviewRequest pictureReviewRequest, User loginUser) {
        ...
        // 2.判断图片是否存在
        Picture oldPicture = this.getById(loginUser.getId());
        ThrowUtils.throwIf(oldPicture == null, ErrorCode.NOT_FOUND_ERROR);
        // 3.判断图片是否重复审核，已经是审核状态
        if (oldPicture.getReviewStatus().equals(pictureReviewEnum)) {
    ...
    public Integer uploadPictureByBatch(PictureUploadByBatchRequest pictureUploadByBatchRequest, User loginUser) {
        ...
        ThrowUtils.throwIf(count > 30, ErrorCode.PARAMS_ERROR, "抓取数量最多30条");
    ...
    public void deletePicture(long pictureId, User loginUser) {
        ...
        transactionTemplate.execute(status -> {
            ...
            if (spaceId != null) {
                boolean update = spaceService.lambdaUpdate().eq(Space::getId, spaceId).setSql("totalCount = totalCount - 1").setSql("totalSize = totalSize - " + oldPicture.getPicSize()).update();
                ThrowUtils.throwIf(!update, ErrorCode.OPERATION_ERROR, "额度更新失败");
            }
            return true;
        });
    ...
    public List<PictureVO> searchPictureByColor(long spaceId, String picColor, User loginUser) {
        ...
        ThrowUtils.throwIf(space == null, ErrorCode.NOT_FOUND_ERROR, "空间不存在");
        if (!space.getUserId().equals(loginUser.getId())) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "没有空间权限访问");
        }
        ...
        // 4.计算相似度并排序
        // 4. 计算相似度并排序
        List<Picture> sortedPictures = pictureList
                .stream()
